# Applies Bulked and Empowered to party for 3 turns.
import random
from sqlmodel import Field
from models.game.skills.base_skill import BaseSkill
from models.game.entities.base_entity import BaseEntity
from models.game.enums.skills.biker_skills import BikerSkills
from models.game.enums.skill_types import SkillType
from models.game.effects.positive_effects.status_effects import Bulked, Empowered
from models.game.effects.applied_effect import AppliedEffect

class GangGang(BaseSkill):
    name: str = Field(default=BikerSkills.GANG_GANG.value)
    skill_type: str = Field(default=SkillType.BUFF.value)
    description: str = Field(default="Applies Bulked and Empowered to party for 3 turns.")
    cooldown: int = Field(default=6)

    def use(self, user: BaseEntity, targets: list[BaseEntity]) -> None:
        """Use the GangGang skill to apply Bulked and Empowered to the entire party."""
        bulked = Bulked()
        empowered = Empowered()
        bulked_effects: list[AppliedEffect] = bulked.generate_effects(duration=3)  # Duration of 3 turns
        empowered_effects: list[AppliedEffect] = empowered.generate_effects(duration=3)  # Duration of 3 turns
        for target in targets:
            for effect in bulked_effects + empowered_effects:
                target.add_effect(effect)

    def level_up(self) -> None:
        """Level up GangGang - increases duration of effects."""
        self.level += 1
        new_duration = 3 + (self.level - 1) * 1
        Bulked.DURATION = new_duration
        Empowered.DURATION = new_duration
        description = f"Applies Bulked and Empowered to party for {new_duration} turns."
        self.description = description